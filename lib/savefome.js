...
const ytIdRegex = /(?:facebook\.com\/\S*(?:(?:\/e(?:mbed))?\/|watch\?(?:\S*?&?v\=))|face\.be\/)([a-zA-Z0-9_-]{6,11})/

/**
 * Download facebookVideo via savefrom
...
async function yt(url, quality, type, bitrate, server = 'en68') {
  ...
  url = 'https://m.facebook.com/' + ytId[1]
  let res = await post(`https://en.savefrom.net/mates/${server}/analyze/ajax`, {
    url,
    q_auto: 0,
    ajax: 1
  })
  ...
  let res2 = await post(`https://en.savefrom.net/188/mates/${server}/convert`, {
    type: 'facebook',
    _id: id[1],
    v_id: ytId[1],
    ajax: '1',
...
module.exports = {
  ...
  ytIdRegex,
  /**
   * Download Facebook and tiktok Video as Audio via savefrom
   * @param {String} url Facebook Video URL
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  yta(url, resol = '128kbps', server = 'en154') { return yt(url, resol, 'mp3', resol.endsWith('kbps') ? resol.replace(/kbps/g, '') : resol, server) },
  /**
   * DDownload Facebook and tiktok Video as Audio via savefrom
